...
initEverything = (func) => {
	...
	switch (func) {
		...
		case '(1 - i)**2+100*(j - i**2)**2':
			...
			break;
		case 'Math.sin(10*(i**2+j**2))/10 + 0.1':
			startX = -1;
			endX = 1;
			startY = -1;
			endY = 1;
			incrValue = 0.01;
			curFunc = 'Math.sin(10*(xGD[xGD.length - 1]**2+yGD[yGD.length - 1]**2))/10 + 0.1';
			dx = '2 * yCur * Math.cos(10 * (xCur**2 + yCur**2))';
			dy = '2 * xCur * Math.cos(10 * (xCur**2 + yCur**2))';
			initialError = 'Math.sin(10*(x1**2+y1**2))/10 + 0.1';
			meanFunc = 'Math.sin(10*(xMean**2+yMean**2))/10 + 0.1';
			break;
		case 'Math.sin(5*i)*Math.cos(5*j)/5 + 0.2':
			startX = -1;
			endX = 1;
			startY = -1;
			endY = 1;
			incrValue = 0.01;
			curFunc = 'Math.sin(5*xGD[xGD.length - 1])*Math.cos(5*yGD[yGD.length - 1])/5 + 0.2';
			dx = 'Math.cos(5 * xCur) * Math.cos(5 * yCur)';
			dy = '-1 * Math.sin(5 * xCur) * Math.sin(5 * yCur)';
			initialError = 'Math.sin(5*x1)*Math.cos(5*y1)/5 + 0.2';
			meanFunc = 'Math.sin(5*xMean)*Math.cos(5*yMean)/5 + 0.2';
			break;
		default:
			break;
	}
...
updatePlot = () => {
	...
	var run = setInterval(function() {
		...
		if (xGDlength < iterations && breakFlag === false) {
			...
			switch (type) {
				case "basic":
					xCur = xGD[xGD.length - 2];
					yCur = yGD[yGD.length - 2];
					xGD.push(nx);
					yGD.push(ny);
					zGD.push(eval(curFunc));
					xCur = xGD[xGD.length - 1];
					yCur = yGD[yGD.length - 1];
					break;
				case "stochastic":
			...
			console.log(
				'new x: ' + round(xCur, 5) + ' - ' + round(learningRate, 5) + ' * ' + round(eval(dx), 5) + ' = ' + round(nx, 5) + '\n' +
				'new y: ' + round(yCur, 5) + ' - ' + round(learningRate, 5) + ' * ' + round(eval(dy), 5) + ' = ' + round(ny, 5)
			);
...
// Stuff required for the modal
initGuidance(["nothing", "tabs", "nothing", "nothing", "nothing", "nothing", "nothing", "params", "checkboxes", "vectors", "tabs", /* everything after tis part won't be shown */ "simButtons", "title"]);
